...
pub const API = struct {
    ...
    // Extension loader
    pub fn loadGlExtensions(comptime GPA: fn (name: [*c]const u8) callconv(.C) *allowzero anyopaque) !void {
        try loadGlExtensionsForStruct(&extensions, GPA);
    }
    fn loadGlExtensionsForStruct(comptime api: anytype, comptime GPA: fn (name: [*c]const u8) callconv(.C) *allowzero anyopaque) !void {
